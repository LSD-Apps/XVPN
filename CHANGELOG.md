...
- **暗色主题下「版本更新」卡片不跟随主题，还有三个移动端页头也是。**
  界面统一通过 `XV.xxx` 读颜色，而那些是**读取可变调色板的静态 getter**
  （`applyPalette` 换掉当前调色板）。颜色不是编译期常量，因此一个组件能不能跟着
  主题走，取决于重建时它的 `build` 有没有被重新执行。

  设置页把它写成了 `const UpdateCard()`。**const 组件实例在重建时被判定为同一个
  对象**（`identical`），Flutter 直接复用已构建的子树，不会再调它的 `build`——
  于是它一直拿着创建时那套调色板算出来的颜色。同一页其余卡片都是方法调用
  （`_buildXxxCard(...)`），每次重建都是新实例，所以只有它出问题，看起来就像
  「只有版本更新这一张漏了」。

  `MobileHeader` 是同一类：它读 `XV.green` / `XV.dotIdle` / `XV.muted2` 画状态
  圆点，却被三个移动端页面各自 `const MobileHeader(...)` 了一次（设置、分流规则、
  连接）。它只影响页头那一行，因此更不容易被注意到。

  全部改为非 const。新增
  [`app/test/update_card_theme_test.dart`](app/test/update_card_theme_test.dart)：
  亮→暗与暗→亮两个方向都断言卡片**实际画出来的底色**跟着变（拿 `BoxDecoration`
  的颜色比对调色板），再加两条结构断言把「不得再用 const」的原因写进代码库。
  实测把 `const` 加回去，三条都会变红。

  这条纪律值得记下：**凡是读 `XV` 颜色的组件都不能声明 const 实例。** 这类缺陷
...
- **新增系统代理看门狗，保证「没连接或连接失效时网络一定回正」。**
  ...
  ——端口没起来就不该接管，否则会主动制造坏状态。

  它**只在有我们留下的痕迹时才动系统代理**：判断依据是备份，而不是「代理现在是
