...
///      在变慢或已经不响应了。
///   2. **需要拿到「原始结果」用于交叉校验**。要判断一个域名的解析结果是否可信，
///      必须比较直连解析器与隧道内解析器的答案；内核只会给你它自己挑中的那一个。
...
/// bind 0.0.0.0   -> 成功 rcode=0 192.0.2.177（12–13ms）
/// ```
///
/// 症状之所以难查：失败是**瞬时**的而不是等满超时，界面把它记成「解析超时」并
/// 得出「直连解析异常」的结论，于是用户照建议去换解析器——而问题根本不在解析器，
...
class UdpDnsResolver implements DnsResolver {
  ...
  Future<DnsOutcome> query(
    ...
    try {
      ...
      // 「解析超时」，于是界面给出的是「去换个直连解析器」——一个修不好的方向。
      Object? socketError;
...
/// 把域名标签转成 ASCII 字节。
///
/// 刻意**不做 punycode 转换**：规则库里的主流站点本就是 ASCII 形式，
