  ...
  static Map<String, Object?> _inbound(
    ...
    return switch (mode) {
      ...
      InboundMode.tun => <String, Object?>{
        ...
        'auto_route': true,
        // stack 必须用 gvisor：**系统协议栈在这条链路上不转发 TCP**。
        //
        // sing-box 的三个取值不是风格差异，而是「谁来终结连接」：
        //   * `system`：TCP 与 UDP 都交给系统协议栈；
        //   * `gvisor`：两者都在用户态协议栈里终结；
        //   * `mixed`：TCP 交给系统，UDP 交给 gvisor。
        //
        // 真机（Android 16 / 小米）上 `mixed` 的表现是「界面已连接、延迟正常，但
        // 所有网页打不开」——延迟探测走的是内核自身的出站，不经过 TUN，所以它
        // 照常出结果，而用户的流量一个字节都没进来。三条观测同时成立：
        //
        //   1. 浏览器请求 http://1.1.1.1/ 超时，对端的 SS 接入端**从未收到**
        //      这条连接；换成 `gvisor` 后同一请求立刻出现在接入端日志里；
        //   2. `mixed` 下 `/connections` 只有直连的国内连接，隧道侧一列为空、
        //      累计字节不增长（界面长期显示「0 B / 0 B/s」）；`gvisor` 下 TUN 里
        //      的连接带着目标、命中规则与字节数出现在列表里，界面数字跟着动；
        //   3. `mixed` 的内核 debug 日志里只有 DNS（UDP，本就归 gvisor）的收发，
        //      没有任何一条 TCP 的 `outbound connection`。
        //
        // 换成 `gvisor` 后，浏览器能打开非国内站点，DNS（UDP 到 1.1.1.1:53）与
        // TCP 都出现在接入端日志里——TUN 这条路算是真的通了。此前也试过 `system`：
        // UDP 跟着一起失效，TCP 依旧不通，所以三个取值里只剩 `gvisor` 可用。
        //
        // 系统栈为何不行，最可能的原因是它的走法：把 TUN 里的 TCP 交给真实套接字，
        // 而那些套接字的源地址是 TUN 自身的地址（172.19.0.1），回包没有一条能回到
        // 这个进程里（VpnService 的进程内没有把它接回来的路径）。建连阶段就静默
        // 消失，正是上面第 3 条「连错误日志都没有」的样子。
        'stack': 'gvisor',
