...
class CnIpIndex {
  ...
  /// 表现为「交叉校验拿不到结论」，而不是「把索引内的地址误判成索引之外」。
...
/// 地址的地理归属判定。
///
/// 之所以做成一个显式的三态而不是 `bool`：拿不到索引与「确定不在索引内」
...
/// 判断一组地址的地理归属。
///
/// 只要有一个落在索引内就按索引内处理：站点的多线解析经常同时返回
/// 多个索引内的地址，也有返回「索引内 + 索引外 CDN」的混合情况，
/// 后者按索引内处理更符合「直连也许能通」的实际。
