...
class AndroidUpdateInstaller implements UpdateInstaller {
  ...
  Future<UpdateInstallResult> install({
    ...
    final String? status;
    try {
      status = await channel.installApk(apk.path);
...
/// 从安卓更新 zip 里取出 APK，返回落盘后的 APK 文件。
///
/// **落在压缩包旁边**（安卓上是私有缓存的 `updates/downloads/`，见
/// [defaultUpdateStagingDir]）：系统安装器拿到的 URI 必须落在 FileProvider 声明的
/// `cache-path` 之内，否则 `getUriForFile` 会抛异常、安装界面根本打不开——两个
/// 路径分头去拼迟早会分叉，因此这里就用下载目录本身。
///
/// 优先取契约里的名字（`XVPN-<ver>-android-arm64.apk`），名字对不上时退回「包内
/// 唯一的 `.apk` 条目」：上游哪天改了压缩包内部的取名方式，更新不该立刻失效。
/// 出现两个 APK 则明确失败——那时猜哪个都是错的。
Future<File> extractAndroidApk({
  required File archive,
  required String version,
}) async {
  final List<ZipEntry> entries = readZipEntries(archive);
  final String expected = 'XVPN-$version-android-arm64.apk';
  final ZipEntry entry = entries.any((ZipEntry e) => e.name == expected)
      ? entries.firstWhere((ZipEntry e) => e.name == expected)
      : singleZipEntry(
          entries,
          (String name) => name.toLowerCase().endsWith('.apk'),
          what: 'APK',
        );

  final String separator = Platform.pathSeparator;
  final File target = File(
    '${archive.parent.path}$separator${entry.name.split('/').last}',
  );
  await extractZipEntry(archive: archive, entry: entry, destination: target);
  return target;
}
