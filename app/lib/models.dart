...
class AppSettings {
  ...
  ///
  /// 这个字段是界面用的**镜像**，事实在系统里（Windows 上是
  /// `HKCU\...\CurrentVersion\Run`，用户可以随时在「任务管理器 → 启动」里改）。
  /// 每次启动都会由 `AutoStartController.refresh()` 与系统核对一次，因此存档里
  /// 的值不会长期偏离真实状态——见 core/auto_start.dart。
