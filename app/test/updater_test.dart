...
void main() {
  ...
  group('附件选择', () {
    ...
    test('安卓取内含 APK 的 zip，且不会串到 Linux 那个 zip', () {
      // 两个 zip 的结尾都是 `.zip`，只有中间的 `-<平台>-<架构>` 不同：按松散的
      // 结尾匹配会把 Linux 的包当成安卓的安装包下下来。
      ...
      final zip = selectAsset(
        release.assets,
        UpdatePlatform.android,
        release.version,
      );
      expect(zip!.name, 'XVPN-1.1.0-android-arm64.zip');

      // 只有 Linux 的包时**不能**退回去：那是给 Linux 的 bundle，解不出 APK。
      final onlyLinux = <ReleaseAsset>[
        ReleaseAsset(
          name: 'XVPN-1.1.0-linux-x64.zip',
      ...
      expect(
        selectAsset(onlyLinux, UpdatePlatform.android, '1.1.0'),
        isNull,
      );
    });
  ...
  group('安卓安装策略', () {
    ...
    /// 把测试夹具（**真实压缩包**，见 test/fixtures/）复制进暂存目录，模拟
    /// 「已经下载并校验过的更新包」。里面那一个条目叫
    /// `XVPN-0.0.0-android-arm64.apk`——与 `_infoFor` 给的版本号不同，正好顺带
    /// 覆盖「契约名对不上时退回包内唯一的 .apk」这条退路。
    File stagedZip({String fixture = 'android-update-deflate.zip'}) {
      final bytes = File('test/fixtures/$fixture').readAsBytesSync();
      return File(
        '${root.path}${Platform.pathSeparator}XVPN-1.1.0-android-arm64.zip',
      )..writeAsBytesSync(bytes);
    }

    Future<UpdateInstallResult> installWith(
      _FakeApkChannel channel, {
      String fixture = 'android-update-deflate.zip',
    }) {
      ...
      return installer.install(
        ...
        archive: stagedZip(fixture: fixture),
        stagingDir: root,
      );
    }
    ...
    test('先从 zip 里取出 APK，再交给系统安装器', () async {
      ...
      // 交给原生的是**解出来的 APK**，不是那个 zip：系统安装器只接受 APK 文件。
      expect(channel.paths.single, endsWith('XVPN-0.0.0-android-arm64.apk'));
      final extracted = File(channel.paths.single);
      expect(extracted.existsSync(), isTrue);
      // 夹具内容是 120 行重复文本，解出来必须与原始长度一致。
      expect(extracted.lengthSync(), 3360);
      // 落在暂存目录里（安卓上就是 FileProvider 声明的 cache-path 之内）。
      expect(extracted.parent.path, root.path);
    });
    ...
    test('包内没有 APK 时如实失败，而不是把 zip 丢给安装器', () async {
      final channel = _FakeApkChannel('launched');
      final result = await installWith(
        channel,
        fixture: 'android-update-no-apk.zip',
      );
      expect(result, isA<UpdateInstallFailure>());
      expect((result as UpdateInstallFailure).message, contains('无法解出安装文件'));
      expect(channel.paths, isEmpty, reason: '取不出 APK 就不该去调系统安装器。');
    });
