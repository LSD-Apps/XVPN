...
1. [`docs/USER_GUIDE.zh-CN.md`](docs/USER_GUIDE.zh-CN.md) — 用户视角的完整路径；
2. [`README.zh-CN.md`](README.zh-CN.md) 的「这是什么，以及不是什么」——
   产品定位会影响哪些改动是可以接受的；
3. [`docs/LEGAL.md`](docs/LEGAL.md) — 表述与合规边界。
...
// 「解析命中规则集的域名也超时」。这不是推测，是通过「只改 detour、其它不动」
// 的对照实验确认的。
```
...
| 统计里只有一个「本次累计」 | 已拆成「本次连接」总量与统计卡内「约 N% 走隧道」；勿再合并命名 |
...
- **不要把真实会话数据写进代码。** 这条是**曾经真的踩过**的坑：
  ...
  * 需要「看起来像真的」时用保留值：域名 `example.net` / `vpn.example.net`，
    IP 用 RFC 5737 的 `203.0.113.0/24` 与 `198.51.100.0/24`（均为文档保留段），
    密钥用递增字节的 base64（`AQIDBAUGBwgJ…`、`ISIjJCUmJygp…`）；
  ...
  **IP 的边界要划清楚**，否则容易走极端：
  ...
  | 类别 | 处置 |
  | --- | --- |
  | 从真实网络观察到的地址（解析结果、抓到的目标地址、实验记录） | **必须替换**——它是真实会话的痕迹 |
  | 本项目的产品配置（直连 DNS `223.5.5.5` / `119.29.29.29`、兜底远程 DNS `1.1.1.1` / `9.9.9.9`） | 保留——这是程序自己的选择，不是用户数据 |
  | 回环、私网、RFC 5737 文档段 | 保留 |
  | 公共 DNS / 协议探测目标（`8.8.8.8`、`www.gstatic.com`、`www.baidu.com`） | 保留——公共基础设施，不指向任何个人 |

  测试夹具要特别小心：**验证「归属判定逻辑」时用保留段就够了**，不要为了
  「看起来真实」而写真实网段——真实网段的归属会变，写成测试事实迟早过期。
  注意字节数组：DNS 报文测试里地址是以 `[192, 0, 2, 34]` 的形式存在的，
  改了字符串却漏了字节，测试会失败（这条也被踩过）。
  ...
  # 2) 公网地址清单——除了白名单那几类，其余都该是保留段
  git grep -I -o -h -E "([0-9]{1,3}\.){3}[0-9]{1,3}" -- . |
    Sort-Object -Unique
  ```
...
本项目的版权与维护者身份见 [`AUTHORS`](AUTHORS)。**不要**把 AI 助手写成作者或
共同作者。

- Git 的 `Author` / `Committer` 必须是**你本人的**姓名与邮箱（或本仓库维护者账号
  `LSD2024`），不要使用 `Cursor` / `cursoragent@cursor.com` 等工具身份；
- **禁止**在 commit message 里添加
  `Co-authored-by: Cursor <cursoragent@cursor.com>`（或同类助手 trailer）。
  GitHub 的 Contributors 会把这类 trailer 显示成仓库贡献者，这与事实不符；
- 若本地工具默认注入上述 trailer，请在提交前删掉，或关闭相关选项。

历史提交若已带有助手 `Co-authored-by`，需要改写历史并强制推送后，GitHub
贡献者列表才会更新——这属于维护者操作（可用 `scripts/strip-cursor-coauthor.sh`），
**不要**在普通 PR 里重写他人历史。
