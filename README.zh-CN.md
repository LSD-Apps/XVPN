...
- **直连白名单**：把「确实能直连、却被默认规则送进隧道」的域名显式拉出来，两类：
  ...
  两者都不依赖解析结果，因此不受 DNS 污染影响。改动**不需要重连**，十几秒内生效
  （见下一条）。
- **决策热更新**：自动纠正学到的规则、手工改判与白名单开关，都作为**可热更新的
  规则集**投递给运行中的内核（本机回环 HTTP，内核每 10 秒拉取一次）。因此改判
  不需要重连、也不需要重启内核——此前这些改动只在「下一次连接」生效，表现是
  「界面说已纠正、网站照样打不开」。
