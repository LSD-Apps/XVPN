...
*   **排除特定 IP（按 IP 段直连）**：设置 `AllowedIPs` 为“排除某段 IP 后的剩余全球 IP 段”。这需要一份精细的**地区 IP 路由表**（如 `auto-add-routes` 项目提供的），通过计算得出需要代理的 IP 范围。
...
2.  **实现基础分流**：通过配置 `AllowedIPs` 字段，结合一份地区 IP 路由表，即可实现“按 IP 段直连、其余代理”的效果。
...
- 方案一（`AllowedIPs` + 地区 IP 路由表）实现最简单，但只能按 IP 分流：
  同一个 IP 上既有应当直连的服务又有应当代理的服务时无法区分，而且 IP 表需要自己维护。
  这与「傻瓜式、导入即用」的目标冲突。
