...
· 自动分流：国内域名与 IP 直连，其余走隧道。内置 geosite-cn 与 geoip-cn
...
启动自检会分别验证「直连」与「隧道」两条路，DNS 监测会分别给国内解析器
与隧道解析器计时，并交叉校验两边的解析结果是否可信。
...
> 若问卷出现「是否可用于规避网络限制」之类的问题（部分分级机构会问），
...
- [ ] 内容分级问卷已完成（本文第五节）
- [ ] 截图与图标已准备（注意：截图**不要**出现「翻墙」「访问被屏蔽网站」
