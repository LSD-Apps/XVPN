# 文档索引

按读者分开，避免「怎么用」和「怎么改代码」混在一起。

## 用户

| 顺序 | 文档 | 内容 |
| --- | --- | --- |
| 1 | [`USER_GUIDE.zh-CN.md`](USER_GUIDE.zh-CN.md) · [`USER_GUIDE.md`](USER_GUIDE.md) | **主教程** |
| 2 | [`LEGAL.md`](LEGAL.md) | 法律与使用声明 |
| 3 | [`../PRIVACY.md`](../PRIVACY.md) | 隐私 |
| 4 | [`../SECURITY.md`](../SECURITY.md) | 安全报告 |

下载：<https://github.com/LSD-Apps/XVPN/releases>  
仓库首页：[`../README.zh-CN.md`](../README.zh-CN.md) · [`../README.md`](../README.md)

## 进阶（设计与实测）

| 文档 | 内容 |
| --- | --- |
| [`RULES.md`](RULES.md) | 规则集、DNS、自动纠正 |
| [`RESILIENCE.md`](RESILIENCE.md) | 自愈与观测 |
| [`PROTOCOLS.md`](PROTOCOLS.md) | 协议适配与参数坑 |
| [`RESEARCH.md`](RESEARCH.md) | 为何选 sing-box（短结论） |

## 维护者

| 文档 | 内容 |
| --- | --- |
| [`../CONTRIBUTING.md`](../CONTRIBUTING.md) | 开发约定、署名 |
| [`../AUTHORS`](../AUTHORS) | 版权与维护者 |
| [`ANDROID.md`](ANDROID.md) | VpnService + libbox |
| [`RELEASE.md`](RELEASE.md) | 发布工程与合规清单 |
| [`STORE_LISTING.md`](STORE_LISTING.md) | Google Play 材料 |
| [`../NOTICE.md`](../NOTICE.md) | 第三方许可 |

其它：[`../CHANGELOG.md`](../CHANGELOG.md)、[`../LICENSE`](../LICENSE)、[`../THIRD-PARTY-NOTICES.md`](../THIRD-PARTY-NOTICES.md)。
